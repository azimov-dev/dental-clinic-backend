const { Appointment, AppointmentItem, Service, Patient, User, sequelize } = require('../models');
...
exports.createAppointment = async (req, res, next) => {
  ...
  try {
    ...
    for (const it of items) {
      ...
      if (!svc) {
        await t.rollback();
        return res.status(400).json({ message: `Service id ${it.service_id} not found` });
      }
      const qty = it.quantity && parseInt(it.quantity) > 0 ? parseInt(it.quantity) : 1;
  ...
  try {
    ...
    const appointments = await Appointment.findAll({
      ...
      include: [
        ...
        { model: User, as: 'doctor', attributes: ['id','full_name','phone'], where: { role: 'doctor' }, required: false },
        { model: AppointmentItem, as: 'items', include: [{ model: Service, as: 'service' }] }
      ],
